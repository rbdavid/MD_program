...
//		fprintf(logOut, "Re-evaluate velocities every %d steps\n\n", reevalvel);
		...
		fprintf(logOut, "RT = %f kcal mol^-1  \n", RT);   
		fprintf(logOut, "Box dimension: %f Angstroms \n \n", box);

		fprintf(logOut, "Step %d\n", iter);
		fprintf(logOut, "Total Energy (kcal mol^-1) = %f \n", Tot_en);
		fprintf(logOut, "Total Potential Energy (kcal mol^-1) = %f \n", Tot_potential_en);
		fprintf(logOut, "Total Kinetic Energy (kcal mol^-1) = %f \n", Tot_kinetic_en);
		fprintf(logOut, "Instantaneous Temperature (K) = %f \n\n", int_temp);
	} else {
		fprintf(logOut, "Step %d\n", iter);
		fprintf(logOut, "Total Energy (kcal mol^-1) = %f \n", Tot_en);
		fprintf(logOut, "Total Potential Energy (kcal mol^-1) = %f \n", Tot_potential_en);
